...
1. Create a Item.IPlay interface that could be applied to instruments. This should have a play method that returns the sound of the instrument being played as a String.
...
3. Create an Item.Instrument superclass which contains attributes you see as being common to all instruments e.g. material it is made from, colour, type (Brass, String, Woodwind, Keyboard etc). The classes created in step 2 above can then inherit from this Instruments class.

4. Create a Item.ISell interface which has a calculateMarkup method, which returns an int/double, based on the buying price and the selling price.

5. Create classes for items the shop may sell, i.e. which implement the Item.ISell interface. Such items should have the following:
  ...
  Some Item.ISell items will be musical instruments i.e. instruments can be both Item.IPlay and Item.ISell.

6. Create a Shop class, which has a collection of Item.ISell items, called stock. In your shop you should be able to:
  -  add items to stock.
